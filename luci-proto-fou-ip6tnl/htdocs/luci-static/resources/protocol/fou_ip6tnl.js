...
return network.registerProtocol('fou_ip6tnl', {
	getI18n() {
		return _('FOU-encapsulated ip6tnl');
	},
	...
	getPackageName() {
		return 'netifd-proto-fou-ip6tnl';
	},
	...
	renderFormOptions(s) {
		...
		o = s.taboption('general', form.ListValue, 'mode', _('Tunnel mode'), _('Payload encapsulated in the tunnel.'));
		o.value('ip4ip6', _('IPv4-in-IPv6'), _('Carries IPv4 payloads. FOU protocol <code>ipip</code>.'));
		o.value('ip6ip6', _('IPv6-in-IPv6'), _('Carries IPv6 payloads. FOU protocol <code>ipv6</code>.'));
		o.default = 'ip4ip6';
		...
		o = s.taboption('general', form.Value, 'ipproto', _('FOU protocol'), _('Inner protocol number/name registered with the FOU listener. Defaults to the mode: <code>ipip</code> for IPv4-in-IPv6, <code>ipv6</code> for IPv6-in-IPv6.'));
		o.placeholder = 'ipip';
		...
		o = s.taboption('advanced', form.Value, 'mtu', _('MTU'), _('Device MTU, accounting for the tunnel overhead (IPv6 + UDP).'));
		o.datatype = 'max(9200)';
		o.placeholder = '1400';
